...
namespace Tishko.Generators
{
	[Generator]
	public class StaticViewLocator : ISourceGenerator
	{
		...
		public void Execute(GeneratorExecutionContext context)
		{
			...
			foreach (var namedTypeSymbol in receiver.NamedTypeSymbolLocators)
			{
				...
				{
					context.AddSource($"{namedTypeSymbol.Name}_StaticViewLocator.cs",
						SourceText.From(classSource, Encoding.UTF8));
				}
			}
		}

		private static string? ProcessClass(Compilation compilation, INamedTypeSymbol namedTypeSymbolLocator,
			List<INamedTypeSymbol> namedTypeSymbolViewModels)
		{
			...
			foreach (var namedTypeSymbolViewModel in namedTypeSymbolViewModels)
			{
				...
				string classNameViewModel =
					$"{namespaceNameViewModel}.{namedTypeSymbolViewModel.ToDisplayString(format)}";
		...
		private class SyntaxReceiver : ISyntaxContextReceiver
		{
			...
			public void OnVisitSyntaxNode(GeneratorSyntaxContext context)
			{
				if (context.Node is ClassDeclarationSyntax classDeclarationSyntax)
				{
					...
					{
						NamedTypeSymbolLocators.Add(symbol); 
					}
					else if (symbol.Name.EndsWith(ViewModelSuffix) && !symbol.IsAbstract)
					{
						NamedTypeSymbolViewModels.Add(symbol);
					}
				}
			}
		}
